...
public class DNSLookupService {
    ...
    private static DNSNode headNode;  //the node we are searching for at the beginning
    /**
    ...
    private static Set<ResourceRecord> getResults(DNSNode node, int indirectionLevel) {
        ...
        //check if the node we are looking for exists in cache
        if (!cachedResults.isEmpty()) {
            return cachedResults;
        }
        //check if a CNAME RR has been found with the target hostName as hostname
        cachedResults = cache.getCachedResults(new DNSNode(node.getHostName(),RecordType.getByCode(5)));
        //if there is no CNAME, query server with node
        if (cachedResults.isEmpty()) {
            if (rootServer != null) {
                retrieveResultsFromServer(node, rootServer);
                ...
                //check if CNAME has been found
                cachedResults = cache.getCachedResults(new DNSNode(node.getHostName(),RecordType.getByCode(5)));

                if (!cachedResults.isEmpty()) {
                    return cacheHasCname(node, indirectionLevel);
                }
            }
        } else {
            return cacheHasCname(node, indirectionLevel);
    ...
    //for each nameserver, recursively look for a solution
    private static Set<ResourceRecord> cacheHasCname(DNSNode node, int indirectionLevel){
        ...
        DNSNode newNode = null; // initialize a new node

        for(ResourceRecord record : cachedRecords){
            newNode = new DNSNode(record.getTextResult(), node.getType());
        }
     ...
     */
    private static void queryNextLevel(DNSNode node, Set<ResourceRecord> nameservers) throws UnknownHostException {
        Set<ResourceRecord> cached = cache.getCachedResults(node);
        for (ResourceRecord r : cached) {
            if ((r.getType().getCode() == 1 || r.getType().getCode() == 28)) {
                return;
            }
        }
        if (nameservers.iterator().hasNext()) {
            retrieveResultsFromServer(node, InetAddress.getByName(nameservers.iterator().next().getTextResult()));
        }
    }
